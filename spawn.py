...
DEFAULT_FPGA_BITSTREAM="bitfiles/boomchip_wrapper.bit"
LINUX_SOURCE="/scratch/celio/linux-flow"
INITFS_SOURCE="/nscratch/celio/initram"
BMARK_SOURCE="/nscratch/celio/qsub-fpga-initramfs/benchmarks"
OUTPUT_DIR="output"
...
def main():
    ...
    for line in f:
        ...
        print cmd_str,
        generate_init_file(cmd_str, dir_str, initfile)
...
#---------------------
def generate_init_file(cmd_str, dir_str, initfile):
...
def generate_bblvmlinux(bmk_str, dir_str, initfile):
    ...
    target = INITFS_SOURCE + "/" + dir_str
    cmds =""
    cmds += "\n"
    cmds += "cp " + initfile + " " + LINUX_SOURCE + "/profile\n"
    cmds += "cp -r " + BMARK_SOURCE + "/" + dir_str + " " + target + "\n"
    cmds += "cd " + LINUX_SOURCE + "; make\n"
    cmds += "cp " + LINUX_SOURCE + "/bblvmlinux /nscratch/celio/qsub-fpga-initramfs/build/bblvmlinux-" + bmk_str + "\n"
    cmds += "rm -r " + target + "\n"
    print cmds
